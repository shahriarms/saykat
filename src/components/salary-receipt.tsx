...
const CheckPattern = () => (
    <svg width="100%" height="100%" className="absolute inset-0 z-0 opacity-[0.07]">
      <defs>
        <pattern id="check-pattern-bg" patternUnits="userSpaceOnUse" width="40" height="40" patternTransform="scale(1) rotate(0)">
      ...
      <rect width="100%" height="100%" fill="url(#check-pattern-bg)" />
    </svg>
);
  ...
  ({ employee, paymentAmount, paymentDate }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("bg-white p-4 font-sans print:p-0", isBn ? 'font-bangla' : '')}>
        ...
        >
            <CheckPattern />
            <div className="border border-gray-400 p-6 relative z-10 space-y-4">
                ...
                <div className="flex items-start gap-4">
                    <div className="flex-1 space-y-3">
                        <div className="flex items-center gap-4">
                            ...
                            <p className="w-full border-b border-gray-400 font-semibold text-base pb-1">{employee.name}</p>
                        </div>
                        <div className="flex items-center gap-4">
                             <p className="w-full border-b border-gray-400 pb-1 capitalize">{amountInWords}</p>
                             <span className="text-xs text-gray-600 self-end">DOLLARS</span>
                        </div>
                    </div>
                ...
                <div className="flex justify-between items-end pt-6">
                     <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-600">{t('phone_label')}:</span>
                ...
                {/* MICR Line */}
                <div className="text-center font-mono text-sm text-gray-500 pt-4">
